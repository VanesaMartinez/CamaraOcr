...
video.width = 400; //jQuery(window).width();
video.height = 400; //jQuery(window).height();
 ...
 * @returns
 */
export function calculateBrightness(src, pointCount) {
  let dst = new cv.Mat();
  let brightness = 0;
 ...
 */
export function notifyBrightness(brightness) {
...
function invokeWebService() {
  ...
  var url = "http://192.168.0.107:8080/ine";
  xmlhttp.open("POST", url, true);
  xmlhttp.setRequestHeader("Content-type", "application/json");
  /*
  xmlhttp.setRequestHeader(
    "X-Parse-Application-Id",
    "VnxVYV8ndyp6hE7FlPxBdXdhxTCmxX1111111"
  );
  xmlhttp.setRequestHeader(
    "X-Parse-REST-API-Key",
    "6QzJ0FRSPIhXbEziFFPs7JvH1l11111111"
  );
  */
